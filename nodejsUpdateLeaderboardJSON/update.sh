...
COUNTER=$(cat readableData.json | underscore select '.key5participants' --outfmt text) # set counter to the first data page it needs to pull (first page that isnt just 5s)
COUNTER=$(($COUNTER/100))
...
TOTAL=$(cat leaderboard.json | underscore select '.total' --outfmt text) # partical creadit https://stackoverflow.com/questions/17883661/how-to-extract-numbers-from-a-string
TOTAL=$(($TOTAL/100))
